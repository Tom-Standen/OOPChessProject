...
public:
	...
	//methods
	virtual std::set<std::pair<int, int>> moves(const board&, const int&, const int&) = 0; // standard move made by each piece
	...
	bool FirstMove() const { return first_move; } //boolean to determine wether piece has moved (pawn double, rook & king castle)
	void HasMoved() { first_move = false; } //record the fact the piece has moved
};
...
public:
	...
	virtual ~pawn(){}
	std::set<std::pair<int, int>> moves(const board&, const int&, const int&) override; // pawn standard move is one space forward (+8)
};
...
public:
	...
	virtual ~rook(){}
	std::set<std::pair<int, int>> moves(const board&, const int&, const int&) override; // rook standard move is unlimited forward,back, left and right (+8,-8,+1,-1)*(up to 7)
};
...
public:
	...
	virtual ~bishop(){}
	std::set<std::pair<int, int>> moves(const board&, const int&, const int&) override; // bishop standard move is unlimited diagonal (+9,-9,+7,=-7)*(up to 7)
};
...
public:
	...
	virtual ~knight(){}
	std::set<std::pair<int, int>> moves(const board&, const int&, const int&) override; // knight standard move is unique (+17,+10,+15,+6,-17,-15,-10,-6)
};
...
public:
	...
	virtual ~queen(){}
	std::set<std::pair<int, int>> moves(const board&, const int&, const int&) override; // queen standard move is unlimited f,b,l,r and diag (+9,-9,+7,-7,+8,-8,+1,-1)*(up to 7)
};
...
public:
	...
	virtual ~king(){}
	std::set<std::pair<int, int>> moves(const board&, const int&, const int&) override; // king standard move is one space f,b,l,r and diag (+9,-9,+7,-7,+8,-8,+1,-1)
};
